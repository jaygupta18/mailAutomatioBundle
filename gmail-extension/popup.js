...
async function handleProcessEmails() {
    ...
    try {
        ...
        if (data.success) {
            // Update statistics
            updateStats(data.processed_count || 1, true);
        ...
        processButton.textContent = errorMessage;
        setTimeout(() => {
            processButton.textContent = '🚀 Process Emails Now';
            processButton.disabled = false;
        }, 3000);
...
function handleSettings() {
    try {
        ...
        alert('Settings page not available. Please check the extension configuration.');
    }
}
